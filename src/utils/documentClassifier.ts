...
export type MIRAChannel = "memory" | "identity" | "relationship" | "agent" | "unknown";

export interface MIRAScores {
  memory: number;
  identity: number;
  relationship: number;
  agent: number;
...
function baseScores(): MIRAScores {
 ...
 */
function applyFilePathHeuristics(scores: MIRAScores, filePath: string) {
 ...
 */
function applyContentHeuristics(scores: MIRAScores, paragraph: string) {
  const t = norm(paragraph);

  // ===== IDENTITY PATTERNS =====
  // Strong identity markers
  if (/\bi am\b|\bi'm\b/.test(t)) scores.identity += 3;
  if (/\bmy name is\b|\bcall me\b/.test(t)) scores.identity += 4;
  if (/\bi believe\b|\bi think\b|\bmy values?\b/.test(t)) scores.identity += 2;
  if (/\bwhat matters to me\b|\bwho i am\b/.test(t)) scores.identity += 3;
  
  // Self-description patterns
  if (/\bi feel like\b|\bpart of me\b|\bmy personality\b/.test(t)) scores.identity += 2;
  if (/\bas a person\b|\bmy identity\b/.test(t)) scores.identity += 3;

  // ===== RELATIONSHIP PATTERNS =====
  // Strong relational markers
  if (/\bwe\b.*\b(are|were|have|share|do)\b/.test(t)) scores.relationship += 3;
  if (/\bour relationship\b|\bbetween us\b|\bhow we are\b/.test(t)) scores.relationship += 4;
  if (/\byou and (i|me)\b|\b(you|i) and (you|me)\b/.test(t)) scores.relationship += 2;
  
  // Interaction patterns
  if (/\bwhen you\b|\bwhen we\b/.test(t)) scores.relationship += 2;
  if (/\bargument\b|\bfight\b|\bmake up\b|\brepair\b|\bconflict\b/.test(t)) scores.relationship += 3;
  if (/\btrust\b|\battachment\b|\bbond\b/.test(t)) scores.relationship += 2;

  // ===== AGENT/BEHAVIORAL PATTERNS =====
  // Preferences and behaviors
  if (/\bi prefer\b|\bi like to\b|\bi tend to\b|\bi usually\b/.test(t)) scores.agent += 3;
  if (/\bi avoid\b|\bi don't like\b|\bi won't\b|\bi hate\b/.test(t)) scores.agent += 3;
  if (/\bmy boundary\b|\bmy boundaries\b|\bmy limits?\b/.test(t)) scores.agent += 4;
  if (/\bi need\b|\bi want\b|\bi expect\b/.test(t)) scores.agent += 2;
  
  // Response patterns
  if (/\bwhen I feel\b|\bwhen I'm triggered\b|\bi respond by\b/.test(t)) scores.agent += 4;
  if (/\bmy habit\b|\bmy routine\b|\bmy pattern\b/.test(t)) scores.agent += 3;
  ...
  if (/\blast (week|month|year)\b|\bago\b/.test(t)) scores.memory += 2;
  
  // Date patterns
  if (/\b(january|february|march|april|may|june|july|august|september|october|november|december)\b/.test(t)) {
    scores.memory += 2;
  }
  if (/\b(19|20)\d{2}\b/.test(t)) scores.memory += 1; // year
  
  // Event markers
  if (/\b(first|last) time\b|\bthe time when\b|\bthat day\b/.test(t)) scores.memory += 3;
  if (/\bremember when\b|\bdo you remember\b/.test(t)) scores.memory += 4;
  if (/\bhappened\b|\boccurred\b|\btook place\b/.test(t)) scores.memory += 2;
  ...
  if (scores.identity > 0 && scores.relationship > 0) {
    ...
    if (/\bwe\b/.test(t)) scores.relationship += 1;
    // "I am" statements lean identity
    if (/\bi am\b/.test(t)) scores.identity += 1;
  }
 ...
 */
export function classifyParagraphToMIRA(
  paragraph: string,
  filePath: string,
  docBias?: Partial<MIRAScores>
): { miraType: MIRAChannel; scores: MIRAScores } {
  ...
  const entries: [MIRAChannel, number][] = [
  ...
  const miraType: MIRAChannel = 
    bestScore > 0 && (bestScore - secondScore >= 1) ? bestType : "unknown";
