...
function setPhase(jobId: string, phase: string, progress: number) {
...
export async function runJob(
  jobId: string,
  jobDir: string,
  config?: { databaseUrl?: string; embedderUrl?: string }
): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[Job ${jobId.slice(0, 8)}] Pipeline done — ` +
    ...
    if (!DATABASE_URL || !EMBEDDER_URL) {
      ...
      fs.writeFileSync(
        path.join(outDir, "persona_blocks.json"),
        JSON.stringify(result.personaBlocks, null, 2)
      );
      fs.writeFileSync(
        path.join(outDir, "human_blocks.json"),
        JSON.stringify(result.humanBlocks, null, 2)
      );
      fs.writeFileSync(
        path.join(outDir, "archival_memories.json"),
        JSON.stringify(result.archivalMemories, null, 2)
      );
      fs.writeFileSync(
