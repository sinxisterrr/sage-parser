...
function isClaudeExport(parsed) {
    return (Array.isArray(parsed) &&
        parsed.length > 0 &&
        parsed[0].chat_messages !== undefined &&
        parsed[0].uuid !== undefined);
}
function convertClaudeToExportRoot(conversations) {
    const mapping = {};
 ...
 */
function detectRoleFromFilename(filePath) {
    ...
    if (assistantNames.length && matchesAny(assistantNames))
        return "assistant";
    if (userNames.length && matchesAny(userNames))
        return "user";
    return "document";
}
/**
 ...
 */
function chunkIntoParagraphs(text) {
 ...
 */
function chunkTextByLines(text, fileName) {
...
function convertTextToExportRoot(text, sourceFile) {
    const mapping = {};
...
export function extractThreads(root) {
    ...
    function walkIterative(startId) {
        const out = [];
        const stack = [startId];
