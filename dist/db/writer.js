...
export class DatabaseWriter {
      ...
      `);
          ...
          `, [table]);
            ...
            for (const m of columnMigrations) {
                await client.query(`ALTER TABLE ${m.table} ADD COLUMN IF NOT EXISTS ${m.column} ${m.type}`).catch(() => { }); // ignore if already exists (older PG without IF NOT EXISTS)
            }
    ...
    async writePersonaBlocks(blocks, embeddings) {
        const client = await this.pool.connect();
        try {
            ...
            for (let i = 0; i < blocks.length; i++) {
                ...
                await client.query(`INSERT INTO persona_blocks
    ...
    async writeHumanBlocks(blocks, embeddings) {
        const client = await this.pool.connect();
        try {
            await client.query("BEGIN");
            for (let i = 0; i < blocks.length; i++) {
                ...
                await client.query(`INSERT INTO human_blocks
           (id, label, content, mira_type, average_weight, min_weight, max_weight,
            message_count, embedding)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
                    id,
    ...
    async writeArchivalMemories(memories, embeddings) {
        ...
        for (let batchStart = 0; batchStart < total; batchStart += BATCH_SIZE) {
            ...
            try {
                ...
                for (let i = batchStart; i < batchEnd; i++) {
                    ...
                    await client.query(`INSERT INTO archival_memories
