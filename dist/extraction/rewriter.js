...
function compressMeaning(text) {
    ...
    const scored = sentences
        .map(s => ({
        text: s,
        score: (s.match(/\b(love|want|fear|feel|need|truth|memory|remember|important|trust|hurt|choose)\b/i)?.length ?? 0)
...
//--------------------------------------------------------------
export function rewriteFragment(raw) {
    const source = raw;
...
// 5. Batch rewrite
//--------------------------------------------------------------
export function rewriteBatch(list) {
